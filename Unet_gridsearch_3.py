...
def bce_dice_loss3(y_true, y_pred):
        loss = a*binary_crossentropy(y_true, y_pred) + b*dice_loss(y_true, y_pred) + c*focal_loss(y_true, y_pred)# + d*jacard_coef_loss(y_true, y_pred)
        return loss / 3.0

def bce_dice_loss4(y_true, y_pred):
        loss = a*binary_crossentropy(y_true, y_pred) + b*dice_loss(y_true, y_pred) + c*focal_loss(y_true, y_pred) + d*jacard_coef_loss(y_true, y_pred)
        return loss / 4.0
...
#%%
#np.save("X_train_ISIC2018.npy", X_train)
#np.save("y_train_ISIC2018.npy", y_train)
#np.save("X_test_ISIC2018.npy", X_test)
#np.save("y_test_ISIC2018.npy", y_test)
...
a_values = [0.2, 0.4, 0.6, 0.7, 1.0]
b_values = [0.7, 0.8, 0.9, 1.0]
c_values = [0.5, 0.6, 0.7, 1.0]
d_values = [0.1, 0.2, 0.3, 1.0]
...
lines=[]
highestIoU=0
for index_i, i in enumerate(a_values):
    for index_j, j in enumerate(b_values):
        for index_k, k in enumerate(c_values):
                a=i
                b=j
                c=k
                ...
                model_jacard.compile(optimizer = 'adam', loss = [bce_dice_loss3],
                ...
                f1_result = f1_score(y_test_flatten, predictions_flatten)
                ...
                print("F-1 Score for testing dataset is: ", str(f1_result))
                print("Recall for testing dataset is: ", recall_result)
                print("Precision Score for testing dataset is: ", precision_result)
                if highestIoU<mean_IoU:
                    highestIoU=mean_IoU
                    a_max=a
                    b_max=b
                    c_max=c
                currentResults=[]
                currentResults.append(i)
                currentResults.append(j)
                currentResults.append(k)
                currentResults.append(mean_IoU)
                currentResults.append(f1_result)
                currentResults.append(recall_result)
                currentResults.append(precision_result)
                current=", ".join(str(x) for x in currentResults)
...
with open('Loss_compound_3.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
...
c=c_max
#d=d_max
model_jacard.compile(optimizer = 'adam', loss = [bce_dice_loss3], metrics = [dice_coef, 'accuracy'])
