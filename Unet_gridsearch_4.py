...
for index_i, i in enumerate(a_values):
    for index_j, j in enumerate(b_values):
        for index_k, k in enumerate(c_values):
            for index_m, m in enumerate(d_values):
                a=i
                b=j
                c=k
                d=m
                ...
                model_jacard.compile(optimizer = 'adam', loss = [bce_dice_loss4],
...
with open('Loss_compound_4.txt', 'w') as f:
    for line in lines:
        f.write(line)
        f.write('\n')
...
model_jacard.compile(optimizer = 'adam', loss = [bce_dice_loss4], metrics = [dice_coef, 'accuracy'])
